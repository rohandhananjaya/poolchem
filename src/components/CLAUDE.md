...
## visits/
`VisitCard` (dashboard list item) · `WaterReadingInput` (test-strip scan button = "coming soon") · `ChemicalRecommendations` · `AddChemicalDialog` (manual chemical entry popup; `onAdd(VisitChemical)`) · `WaterHealthGauge` · `VisitNotes` (voice recording = "coming soon") · `CancelVisitDialog` (reason-select + custom-reason confirmation UI) · `VisitChargeCard` (card-present payment at the equipment pad, Epic 1: amount entry + charge button calling `chargeVisitAction`, paid state; hidden on cancelled visits, read-only on another tech's in-progress visit)
