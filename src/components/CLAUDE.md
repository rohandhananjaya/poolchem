...
## pools/
`AddPoolDialog` / `EditPoolDialog` / `DeletePoolDialog` (create/edit/delete forms, post to `pools/actions.ts`) · `PoolRow` (list card, opens `EditPoolDialog`) · `PoolActions` (edit/delete menu) · `PoolsFilters` (search/status bar) · `PoolAnalysis` (trend/detail view for `pools/[poolId]`) · `ImportPoolsDialog` (CSV bulk-import: file picker, client-side preview, per-row skip/reason summary; gated by `canImportExport`) · `ExportPoolsButton` (CSV download via client-side Blob; gated by `canImportExport`) — both import/export gated behind the `csv_import` plan feature (`checkFeatureAccess`), showing a locked upsell hint when unavailable.
