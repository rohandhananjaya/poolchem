...
## pools/
`AddPoolDialog` / `EditPoolDialog` / `DeletePoolDialog` (create/edit/delete forms, post to `pools/actions.ts`) · `AddPoolDialog` takes optional `properties` / `defaultPropertyId` / `trigger` props — renders a Location `<select>` (native, `ScheduleVisitForm` inputClasses idiom) when the company has properties, and is reused as the "New pool" trigger on property cards; `EditPoolDialog` likewise takes `properties` + the pool's `propertyId` for move/detach in one edit · `PoolRow` (list card, opens `EditPoolDialog`; shows a muted property-name pill when grouped) · `PoolActions` (edit/delete menu — currently unused) · `PoolsFilters` (search/status bar) · `PoolAnalysis` (trend/detail view for `pools/[poolId]`; shows the pool's tech-scan QR inline in the pool info card — right column under the Homeowner Dashboard button — with a copy-link button, no reissue) · `ImportPoolsDialog` (CSV bulk-import: file picker, client-side preview, per-row skip/reason summary; gated by `canImportExport`) · `ExportPoolsButton` (CSV download via client-side Blob; gated by `canImportExport`) — both import/export gated behind the `csv_import` plan feature (`checkFeatureAccess`), showing a locked upsell hint when unavailable.

## properties/
`AddPropertyDialog` / `EditPropertyDialog` (create/edit forms, post to `properties/actions.ts`) · `DeletePropertyDialog` (confirm-name-match; warns pools stay and become ungrouped — SetNull detach, never cascade) · `PropertyRow` (CardRow: name, address, mono `N pool(s)` count badge, trailing `PropertyActions` dropdown) · `PropertyActions` (edit/delete dropdown menu) · `PropertyPoolsManager` (per-property attach/detach surface: attached-pool chips with detach ×, "Add pools" dialog with a searchable scrollable checkbox list of the company's **ungrouped** pools, and a "New pool" button that opens `AddPoolDialog` pre-bound to this property via `defaultPropertyId`; submits `setPoolPropertyAction` per pool) — `properties` prop (the `{id,name}` list) is drilled from the page for the Location select.

## visits/
`VisitCard` (dashboard list item) · `WaterReadingInput` (test-strip scan button = "coming soon") · `ChemicalRecommendations` · `AddChemicalDialog` (manual chemical entry popup; `onAdd(VisitChemical)`) · `WaterHealthGauge` · `VisitNotes` (voice recording = "coming soon") · `CancelVisitDialog` (reason-select + custom-reason confirmation UI) · `SyncStatusBadge` (presentational offline-sync indicator — pending/syncing/failed/offline/synced with tone colors + lucide icon + optional counts; driven by `useVisitSyncStatus` in the visit form) · `VisitPhotoCapture` (per-body photo capture; props `visitId`, `serviceVisitPoolId`, `photos: {id,url}[]`, `disabled?` — sr-only `type="file"` with `capture="environment"` + `<label asChild>` Add button, thumbnail grid with X delete overlay, empty-state hint; uploads via `uploadVisitPhotoAction(visitId, serviceVisitPoolId, fd)` and deletes via `deleteVisitPhotoAction(visitId, photoId)` from `(dashboard)/visits/[visitId]/photo-actions`; validates with `validatePhotoFile` from `@/lib/storage/photo-format`; `disabled` hides add + delete; designed to slot into the per-body card stack, not wired into visit-form yet)
