...
- `offline/page.tsx` — **static** (`dynamic = "force-static"`) offline fallback page. Precached via `APP_SHELL_PRECACHE` and served by the SW navigation fallback (`buildFallbacks()` in `lib/offline/sw-policy.ts`) when a full document load fails offline; also reachable at `/offline` when online. Renders `<OfflineRouteView>` — when the SW serves it for a `/pools` navigation (address bar still reads `/pools`), the cached pools snapshot shows instead of the generic copy. No auth (the SW-served copy can't call `requireTech()`); tenant-scoped data renders client-side (`OfflineStatus`, `getPoolCache`), scoped by the company id `<IdleRoutePrefetch>` mirrored into localStorage
