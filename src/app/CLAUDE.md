...
- `admin/packages/` — plan catalog CRUD (pricing/features/sort order) + platform trial-length setting + per-company plan/status override + platform payment-provider toggles + **fee-per-transaction billing toggle** (`updateFeeBasedBillingAction`, migrates all companies to `FEE_BASED` when enabled). `actions.ts`: `createPackageAction`, `updatePackageAction`, `deletePackageAction`, `adminSetCompanyPackageAction`, `updateTrialDaysAction`, `updatePaymentSettingsAction`, `updateFeeBasedBillingAction`
