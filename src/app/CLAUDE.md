...
- `pools/`, `pools/[poolId]/` — pool list (`getPoolsPaginated`, CSV import/export, filters) + `PoolAnalysis` detail/trend page (`requireTech`; `getPoolById` + `getVisitHistory` + `getWaterHealthScore`; shows the pool's tech-scan QR inline in the pool info card with a copy-link button). The list page also mounts `<PoolsCacheMirror>` so the offline fallback can show the last-observed pools. `actions.ts`: `createPoolAction`, `updatePoolAction`, `deletePoolAction`, `importPoolsAction`, `exportPoolsAction`
