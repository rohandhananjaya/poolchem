...
- `pools/`, `pools/[poolId]/` — pool list (`getPoolsPaginated`, CSV import/export, filters) + `PoolAnalysis` detail/trend page (`requireTech`; `getPoolById` + `getVisitHistory(poolId, user.companyId, 20)` — history tenant-scoped via the `ServiceVisitPool` join rows + `getWaterHealthScore`; shows the pool's tech-scan QR inline in the pool info card with a copy-link button). The list page also mounts `<PoolsCacheMirror>` so the offline fallback can show the last-observed pools. `actions.ts`: `createPoolAction`, `updatePoolAction`, `deletePoolAction`, `importPoolsAction`, `exportPoolsAction`
...
- `properties/` — OWNER-only multi-body grouping UI (`requireOwner`; `getPropertiesByCompany` + `getPoolsByCompany`); list/create/edit/delete Properties + attach/detach pools via a checkbox multi-select of ungrouped pools. `actions.ts`: `createPropertyAction`, `updatePropertyAction`, `deletePropertyAction`, `setPoolPropertyAction` (attach/detach — revalidates both `/properties` and `/pools`)
...
- `visits/[visitId]/` — the visit form. `page.tsx` → `visit-form.tsx` (client), `status-dropdown.tsx`. The page also mounts `<VisitsCacheMirror>` so the offline fallback can show the last-observed visit.
